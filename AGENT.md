...
本项目已接入 CNB 知识库（Knowledge Base），当仓库有代码推送时自动更新。

知识库会自动索引以下文件：
- 所有 `.md` 文件
- 所有 `.json` 数据文件

用户可通过知识库快速检索设计系统相关信息。
...
UI 领域万事通，精通一切 UI 相关知识。角色定义在 `.cnb/settings.yml` 中。

**使用方式**：在 Issue 或 PR 评论中 @UI哥

**核心能力**：

1. **UI 技术选型推荐**：根据上下文推荐合适的 UI 技术方案
2. **UI 知识解答**：回答任何 UI 相关问题
3. **设计系统收藏管理**：推荐优秀设计系统时，自动评估并加入收藏
4. **UI 建议**：给出项目地址时，分析并给出 UI 改进建议

### 定时任务 NPC

通过 `.cnb.yml` 配置了每周一早上 8:00 的定时维护任务（`crontab: 0 8 * * 1`）。

定时任务会：
1. 执行 `bun run update:sources` 更新原始来源数据
2. 执行 `bun run update:readme` 同步 README
3. 执行 `bun run build` 验证构建
4. 调用 UI哥 NPC 检查是否有可提升到主数据的条目

## NPC 流水线

NPC 事件流水线配置在 `.cnb.yml` 中，使用 NPC 运行时镜像（Dockerfile 构建）。

当 NPC 镜像需要更新时，推送到 main 分支会自动构建并推送镜像。
